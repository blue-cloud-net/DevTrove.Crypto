#!/bin/bash

# CRL 测试素材生成脚本（tongsuo）
# 生成 RSA / EC / DSA / SM2 四种算法的 CRL 测试素材，供"素材双方解析对照"测试使用。
# 输出:
# - tests/data/crls/test.crl : RSA，颁发者 CN=Test CRL CA，发行 serial 1111(keyCompromise)/2222(superseded)
# - tests/data/crls/ec.crl   : EC P-256，颁发者 CN=EC Test CRL CA，发行 serial EC01(keyCompromise)
# - tests/data/crls/dsa.crl  : DSA，颁发者 CN=DSA Test CRL CA，发行 serial D500(keyCompromise)
# - tests/data/crls/sm2.crl  : SM2，颁发者 CN=SM2 Test CRL CA，发行 serial 5E01(keyCompromise)，使用 SM3 摘要
# 吊销时间由生成时刻决定，测试仅断言条目的存在与序列号

set -e

# 唯一外部工具：tongsuo
: "${TONGSUO_PATH:=/opt/tongsuo/bin/tongsuo}"
if [[ ! -x "$TONGSUO_PATH" ]]; then
  echo "tongsuo not found at $TONGSUO_PATH (override with TONGSUO_PATH)" >&2
  exit 127
fi
TONGSUO_BIN="$TONGSUO_PATH"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(dirname "$SCRIPT_DIR")"
KEYS_DIR="$PROJECT_ROOT/tests/data/keys"
OUTPUT_DIR="$PROJECT_ROOT/tests/data/crls"

echo "====================================="
echo "CRL 测试素材生成"
echo "====================================="

mkdir -p "$OUTPUT_DIR"
if [ ! -f "$KEYS_DIR/rsa-2048-pkcs1.pem" ] || [ ! -f "$KEYS_DIR/sm2-pkcs8.pem" ]; then
    "$SCRIPT_DIR/generate-test-keys.sh"
fi

# 为给定算法/密钥生成一份 CRL（复用最小 CA 数据库）
# $1: 算法标识（用于临时目录命名） $2: CA 私钥 $3: 输出文件 $4: CA 颁发者 DN
# $5: 吊销条目 "serial:reason" 列表（空格分隔）
# $6: CA 配置 default_md（sha256 或 sm3；默认 sha256）
gen_crl() {
    local algo="$1"
    local key="$2"
    local out="$3"
    local issuer="$4"
    local revoked="$5"
    local default_md="${6:-sha256}"

    local temp_dir="$OUTPUT_DIR/temp_ca_db_${algo}"
    rm -rf "$temp_dir"
    mkdir -p "$temp_dir/newcerts"
    touch "$temp_dir/index.txt"
    echo "unique_subject = no" > "$temp_dir/index.txt.attr"
    echo "01" > "$temp_dir/serial"

    # CA 配置
    cat > "$temp_dir/ca.cnf" <<EOF
[ ca ]
default_ca = CA_default

[ CA_default ]
dir = $temp_dir
database = $temp_dir/index.txt
new_certs_dir = $temp_dir/newcerts
certificate = $temp_dir/ca.crt
private_key = $temp_dir/ca.key
serial = $temp_dir/serial
default_md = $default_md
default_days = 365
default_crl_days = 30
policy = policy_any

[ policy_any ]
commonName = supplied
EOF

    # CA 自签名证书
    ${TONGSUO_BIN} req -x509 -new -key "$key" \
        -out "$temp_dir/ca.crt" -days 3650 -set_serial 0xCA01 \
        -subj "$issuer" 2>/dev/null

    # CA 私钥（供 tongsuo ca 使用）
    cp "$key" "$temp_dir/ca.key"

    # 生成叶子证书并吊销
    for entry in $revoked; do
        serial="${entry%%:*}"
        reason="${entry##*:}"
        cert="$temp_dir/cert-$serial.pem"
        csr="$temp_dir/cert-$serial.csr"

        ${TONGSUO_BIN} req -new -key "$key" \
            -out "$csr" -subj "/C=CN/O=CRL Org/CN=revoked-$serial" 2>/dev/null

        # 指定序列号签发
        echo "$serial" > "$temp_dir/serial"
        ${TONGSUO_BIN} ca -config "$temp_dir/ca.cnf" -batch -in "$csr" -out "$cert" \
            -startdate 20250101000000Z -enddate 20350101000000Z 2>/dev/null

        ${TONGSUO_BIN} ca -config "$temp_dir/ca.cnf" -batch -revoke "$cert" \
            -crl_reason "$reason" 2>/dev/null
    done

    # 生成 CRL
    ${TONGSUO_BIN} ca -config "$temp_dir/ca.cnf" -gencrl -out "$out" 2>/dev/null

    rm -rf "$temp_dir"
}

# 1. RSA CRL
echo "[1/4] RSA CRL..."
gen_crl "rsa" "$KEYS_DIR/rsa-2048-pkcs1.pem" "$OUTPUT_DIR/test.crl" \
    "/C=CN/O=CRL Org/CN=Test CRL CA" "1111:keyCompromise 2222:superseded"
echo "  ✓ test.crl"

# 2. EC CRL
echo "[2/4] EC CRL..."
gen_crl "ec" "$KEYS_DIR/ec-p256-pkcs8.pem" "$OUTPUT_DIR/ec.crl" \
    "/C=CN/O=EC CRL Org/CN=EC Test CRL CA" "EC01:keyCompromise"
echo "  ✓ ec.crl"

# 3. DSA CRL
echo "[3/4] DSA CRL..."
gen_crl "dsa" "$KEYS_DIR/dsa-2048-private.pem" "$OUTPUT_DIR/dsa.crl" \
    "/C=CN/O=DSA CRL Org/CN=DSA Test CRL CA" "D500:keyCompromise"
echo "  ✓ dsa.crl"

# 4. SM2 CRL（使用 SM3 摘要签名；序列号须为纯 hex 数字以便 tongsuo ca 解析为 ASN.1 INTEGER）
echo "[4/4] SM2 CRL..."
gen_crl "sm2" "$KEYS_DIR/sm2-pkcs8.pem" "$OUTPUT_DIR/sm2.crl" \
    "/C=CN/O=SM2 CRL Org/CN=SM2 Test CRL CA" "5E01:keyCompromise" "sm3"
echo "  ✓ sm2.crl"

echo "完成。"
