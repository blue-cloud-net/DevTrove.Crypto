...
|---|---|---|
| **L1** | Protocol version matrix, cipher-suite matrix, extension fingerprint, server-sent certificate chain, negotiated groups & signature algorithms | ✅ Scheduled (Phase 3 in the parent roadmap) |
| **L2** | L1 + A–F grading, client-simulation compatibility matrix, ALPN/HTTP2, certificate transparency, DNS CAA | ✅ Scheduled (Phase 4 in the parent roadmap) |
| **L3** | L2 + vulnerability probing (Heartbleed, CCS Injection, ROBOT, Ticketbleed, etc.) | ❌ Only ROBOT (see §9.3) |

The parent `DevTrove` roadmap owns the **schedule**; this document owns the **design**.
...
| **Pure-managed** | Cross-platform consistent; compiles into WebAssembly (for certificate parsing etc.) |
...
So RFC 8998 **theoretically supports a full handshake** (not only detection). The roadmap item for verification is in the parent repo; if confirmed feasible, SM2-TLS 1.3 lands in the `TlsProbe` engine.
...
The consuming application maps these models to its own contract DTOs.
...
The consuming application owns these controls; this library provides the building blocks (raw probing capability) and trusts the host to apply them.
...
| **NTLS fixture replay** | Capture ServerHello / Certificate / ServerKeyExchange bytes from public ShangMi sites as static fixtures; verify parse + verdict logic (no Tongsuo dependency) |
...
| [roadmap.md](roadmap.md) | Phase plan for the engine |
